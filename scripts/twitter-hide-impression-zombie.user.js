...
(() => {
  const mutationObserver = new MutationObserver(() => {
    ...
    const currentTweetScreenname = currentTweet
      ? currentTweet.querySelector("[data-testid='User-Name'] a").href.split(
        "/",
      ).at(-1).trim()
      : undefined;
    ...
    if (currentTweet) {
      document.querySelectorAll(
        ...
        const screenname = userNameElem
          ? userNameElem.href.split(
            "/",
          ).at(-1).trim()
          : undefined;
        ...
        if (userNameElem && /[\p{scx=Arabic}]+/u.test(userNameElem.innerText)) {
          elem.style = "display: none;";
        }
      });
    }
